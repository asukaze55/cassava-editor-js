...
class Grid {
  ...
  endEditing() {
    this.element.focus();
    // Wait until #onFocusOut finishes.
  ...
  async moveTo(x, y) {
    ...
    cellNode.focus();
    setTimeout(() => getSelection().setBaseAndExtent(
 ...
 * @returns {StaticRange}
 */
function getComposedRange(shadow) {
  return getSelection().getComposedRanges({shadowRoots: [shadow]})[0];
}
